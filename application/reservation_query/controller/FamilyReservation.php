...
class FamilyReservation extends Base
{
    ...
    public function index()
    {
        ...
        if($submit != 2){
            ...
            $appendList = $appendModel -> getFamilyAppendList($project_code,$whereSql,$hr_id,$page,$son_str);
            ...
            //员工列表
                ...
                //获取自费加项包信息
                    ...
                    $list[$key]['belongs_staff'] = '';
                //获取所属员工信息
                    if (isset($value['order_type_code']) && ($value['order_type_code'] == 2 || $value['order_type_code'] == 3 ) ){
                        $belongsStaffInfo = $this->getBelongsStaffInfo($project_code,$value['append_user_code']);
                        $list[$key]['belongs_staff'] = $belongsStaffInfo['u_name'] ? $belongsStaffInfo['u_name'] : "";
                    }

                    $data[$key]['id'] = $key+1;
                    $data[$key]['append_name'] = $value['append_name'];
                    $data[$key]['belongs_staff'] = $list[$key]['belongs_staff'];
        ...
        //预约人数统计
            ...
            $this->assign('u_son_code',$u_son_code);
            ...
            $son_str = $this->son_str;
            $appendModel = new SerAppend();
            $appendList = $appendModel -> getFamilyAppendListAll($project_code,$whereSql,$hr_id,$son_str);
            $data = [];
            if (!empty($appendList)) {
            //员工列表
                $list = $appendList;
                ...
                //获取所属员工信息
                    ...
                    $data[$key]['report_upload_time_days'] = $list[$key]['report_upload_time_days'];
                }

            }

            $headArr = ["编号","姓名","所属员工","性别","婚姻","证件类型","证件号码","手机号码","预约单号","订单号","预约门店","体检日期","体检套餐","自费加项包",
            "绑定加项包","预约状态","报告状态","报告处理时间"];
            $width = array(10,30,30,20,20,20, 20,20,20,20,20, 20,20,30,40,10, 20,20);
            $columnType = null;
            $excel = new Excel();
            $excel->output_excel('家属预约信息查询',$headArr,$data,$width);
        }
    ...
    * 获取所属员工信息
    */
    public  function getBelongsStaffInfo($project_code,$append_user_code = ''){

        if (empty($project_code) || empty($append_user_code)){
            return NULL;
        }
        ...
        $staffInfo = $appendModel->getStaffInfo($project_code,$append_user_code);

        //免费家属
        if(isset($staffInfo['u_parent_id']) && $staffInfo['u_parent_id'] !== '0'){
            $staffInfo = $appendModel->getStaffInfo($project_code,$staffInfo['u_parent_id']);
        }
